...
const _getCurrentSeats = async (
  ...
  return bookings.map((booking) => {
    const filteredAttendees = booking.attendees.filter((attendee) => !teamMembers?.includes(attendee.email));
...
const _getUserAvailability = async function getUsersWorkingHoursLifeTheUniverseAndEverythingElse(
  ...
  const teamMemberEmails = eventType?.team?.members.map((member) => member.user.email) || [];
  /* Current logic is if a booking is in a time slot mark it as busy, but seats can have more than one attendee so grab
    current bookings with a seats event type and display them on the calendar, even if they are full */
  let currentSeats: CurrentSeats | null = initialData?.currentSeats || null;
  if (!currentSeats && eventType?.seatsPerTimeSlot) {
    currentSeats = await getCurrentSeats(eventType.id, dateFrom, dateTo, isTeamEvent, teamMemberEmails);
  }
